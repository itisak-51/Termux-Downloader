...
#  Interactive file selection for both modes, size preview (GDrive), pure Bash.
# ============================================================================
...
# Pure Bash human‑readable size (no bc required)
human_size() {
    local bytes=$1
...
download_with_aria2() {
    ...
    if [[ $? -eq 0 ]]; then
        ...
        log "FAILED: $filename (will retry when script is re-run)"
        return 1
    fi
}
...
select_files() {
    # Expects arrays: file_url, file_name, file_size (size can be empty)
    local index=$1
    printf "\n%4s  %-60s %10s\n" "No." "Filename" "Size"
    echo "--------------------------------------------------------------"
    ...
    for idx in "${chosen_indices[@]}"; do
        download_with_aria2 "${file_url[$idx]}" "${file_name[$idx]}"
    done
}

# ----------------------------- Case 1: links.txt (now interactive) --------
process_links_txt() {
    local input_file="$1"
    ...
    declare -a file_url file_name file_size
    local index=0
    while IFS= read -r line; do
        ...
        # Size not available from direct links without HEAD request
        file_size[index]=""
    done < "$input_file"
...
# ----------------------------- Google Drive Lister Auto-Generation -------
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            if not os.path.exists(CRED_FILE):
                sys.stderr.write("ERROR: credentials.json not found. Get it from Google Cloud Console.\n")
                sys.exit(1)
...
if __name__ == '__main__':
    ...
    for f in items:
        url = f"https://drive.usercontent.google.com/download?id={f['id']}&confirm=t"
        name = f['name']
        size = f.get('size', '0')
...
process_gdrive_folder() {
    ...
    log "Fetching file list from Google Drive folder ID: $folder_id"
